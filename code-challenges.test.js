...
const firstVowel = (str) => {
  ...
  console.log(arr)
  ///// --- using a forloop checking if the element is equal to a vowel, and returning the index of that element if meets the condition.
  // this way ---
  ...
  //// --- or using the builtin method .forEach() to check the value of each element using conditinals and push the index values of the elements that meets the condition into a new array. Than returning the first element of that array.
  let indexArr = []
  arr.forEach((item, index) => {
    item === 'a' || item === 'e' || item === 'i' || item === 'o' || item === 'u' ? indexArr.push(index) : console.log(item)
  })
  return indexArr[0]
}